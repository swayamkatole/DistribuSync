...
public class ZooKeeperLeaderElection implements Watcher {
    ...
    public void start() throws Exception {
        ...
        // Create election root if not exists
        Stat stat = zooKeeper.exists(ELECTION_PATH, false);
        if (stat == null) {
            zooKeeper.create(ELECTION_PATH, new byte[0],
                    ZooDefs.Ids.OPEN_ACL_UNSAFE,
                    CreateMode.PERSISTENT);
        }

        // Create ephemeral sequential node for this scheduler
        currentZNode = zooKeeper.create(
                ELECTION_PATH + "/scheduler_",
                schedulerId.getBytes(),
                ZooDefs.Ids.OPEN_ACL_UNSAFE,
    ...
    private void checkLeadership() throws Exception {
        ...
        if (currentZNode.equals(smallest)) {
            ...
            int index = children.indexOf(
                    currentZNode.replace(ELECTION_PATH + "/", ""));
